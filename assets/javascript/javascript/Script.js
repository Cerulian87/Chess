...
class Chess {
	...
	setDefault() {
		this.info = {
			...
			started: false, // Initializes game start
			ended: false, // Initializes game ending
	...
	async init(callback) {
		...
		// Player ready check - NOTE* Still need to add websockets
	...
	async assignPlayers() {
		...
		return new Promise((resolve) => {
			const player1 = new Player({ username: "Johnathon", id: 1, role: "white" }); // player 1
			const player2 = new Player({ username: "Not Johnathon", id: 2, role: "black" }); // player 2
	...
	start() {
		...
		this.data.board.placePiecesAsDefault(); // Simpy places pieces on the board in the starting position
		this.data.players.forEach((p) => p.startTimer()); // Starts the timer for player 1
	}
	...
	// Function to identify and alert that there is a winner.
	winner() {
		const Winner = this.info.won;
		const CreatePopUp = function () {
			alert(`The winner is ${Winner.data.username}`) ? "" : location.reload(); // Brings up an alert box and refreshes on "okay"
		};
